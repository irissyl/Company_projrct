...
export function usePermission() {
   ...
   * @param value
   */
  function hasMenuByPath(path){
    const permissionStore = usePermissionStoreWithOut();
    const menus = permissionStore.getBackMenuList;
    if(menus && menus.length > 0){
      for(let i=0;i<menus.length;i++){
        if(hasTargetMenupermissionByPath(menus[i],path)){
          return true;
        }
      }
    }
   ...
   * @param path
   */
  function hasTargetMenupermissionByPath(menu:Menu,path){
      if(menu.path == path){
        return true;
      }
      ...
      if(childrenMenu && childrenMenu.length > 0){
        for(let i=0;i<childrenMenu.length;i++){
          if(hasTargetMenupermissionByPath(childrenMenu[i],path)){
            return true;
          }
        }
      }
  ...
  return { changeRole, hasPermission, togglePermissionMode, refreshMenu, isDisabledAuth,hasMenuByPath };
}
