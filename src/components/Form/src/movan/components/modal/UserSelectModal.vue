...
<template>
    <div>
        <BasicModal v-bind="$attrs" @register="register" title="用户选择" width="1100px" wrapClassName="j-user-select-modal" @ok="handleOk" destroyOnClose @visible-change="visibleChange">
            <a-row>
                <a-col :span="showSelected?18:24">
                    <BasicTable rowKey="userId" :columns="columns" v-bind="getBindValue" :useSearchForm="true" :formConfig="formConfig" :api="getUserList" :searchInfo="searchInfo" :rowSelection="rowSelection" :indexColumnProps="indexColumnProps"></BasicTable>
                </a-col>
...
<script lang="ts">
  ...
  import { useSelectBiz } from '/@/components/Form/src/movan/hooks/useSelectBiz';
  import { useAttrs } from '/@/hooks/core/useAttrs';
  import { selectProps } from '/@/components/Form/src/movan/props/props';
            ...
            componentProps:({formActionType,formModel}) => {
                return {
                    ...
                    onSelect: (options,values) => {
                        console.log("JSelectDept" + JSON.stringify(options) +":" + JSON.stringify(values))
