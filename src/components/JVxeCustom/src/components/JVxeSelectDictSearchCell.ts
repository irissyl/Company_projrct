...
import { JVxeComponent } from '/@/components/movan/JVxeTable/types'
import { dispatchEvent } from '/@/components/movan/JVxeTable/utils'
import { useResolveComponent as rc } from '/@/components/movan/JVxeTable/hooks'
import { useJVxeComponent, useJVxeCompProps } from '/@/components/movan/JVxeTable/hooks'
...
export const DictSearchSpanCell = defineComponent({
  ...
  setup(props: JVxeComponent.Props) {
    ...
    return () => {
      return h('span', {}, [
        filterDictText(innerOptions.value, innerSelectValue.value || innerValue.value),
      ])
    }
  },
})
...
export const DictSearchInputCell = defineComponent({
  ...
  setup(props: JVxeComponent.Props) {
    ...
    const loadData = debounce((value) => {
      ...
      loadDictByKeyword(dict.value, value).then(res => {
    ...
    function renderOptionItem() {
      ...
      options.value.forEach(({ value, text, label, title, disabled }) => {
        optionItems.push(h(rc('a-select-option'), {
          key: value,
          value: value,
          disabled: disabled,
        }, {
...
function useSelectDictSearch(props) {
  ...
  return {
    ...setup,
    loading, innerOptions, innerSelectValue,
    dict, isAsync, options,
  }
}
