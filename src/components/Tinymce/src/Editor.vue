...
<script lang="ts">
    ...
    import { getAbsFileAccessHttpUrl, getFileAccessHttpUrl } from "/@/utils/common/compUtils";
    const tinymceProps = {
        options: {
    ...
    export default defineComponent({
        ...
        setup(props, { emit, attrs }) {
                        ...
                        const uploadSuccess = (res)=>{
                            if (res.success) {
                                if(res.result == 'local'){
                                    ...
                                    let img = getAbsFileAccessHttpUrl(res.result)
                                    console.log("------getFileAccessHttpUrl---",img);
                                    success(img)
                                }
                            }
                        }
            ...
            function handleDone(name: string, url: string) {
                console.log("handleDone=name",name,url)
                const editor = unref(editorRef);
                if (!editor) {
                    return;
                }
